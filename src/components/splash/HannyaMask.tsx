...
// colors), so these are copied from index.css's --color-fg-muted (Slate
// Fog) and --color-accent (Signal Amber) rather than referencing them live.
const GREY = '#5c7a99'
const AMBER = '#ff9f45'
 ...
 * outline both burn from grey to amber as the reveal finishes, capped with
 * an amber glow that blooms in last. Timed to land well inside SplashScreen's
 ...
 *
 * This is deliberately NOT an SVG dasharray/`pathLength` trace (the
 * technique the old wordmark splash used, before it was replaced by this
 * mask): that technique reveals proportionally to *arc length*, and this
  ...
  return (
    <m.svg
      ...
      initial={{
        ...
        filter: 'drop-shadow(0 0 0px rgba(255,159,69,0)) drop-shadow(0 0 0px rgba(255,159,69,0))',
      }}
      animate={{
        clipPath: 'circle(75% at 50% 50%)',
        filter: 'drop-shadow(0 0 14px rgba(255,159,69,0.75)) drop-shadow(0 0 32px rgba(255,159,69,0.45))',
      }}
      transition={{
