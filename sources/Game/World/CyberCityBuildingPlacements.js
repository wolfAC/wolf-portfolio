...
// D4: unique "hero" landmarks — same modular system, deliberately extreme
...
//
// Phase L relocated hero-skylineObservatory (the reserved acid-green beacon,
// the tallest of the three) to stand just outside the hub (Transit Nexus)
// after its home district was removed. Removed outright instead, per direct
// playtesting feedback: at the hub -- the spawn point and busiest, most
// central spot in the city -- its size blocked sightlines for the player.
// The acid-green beacon color (phase-a1-art-direction-brief.md) is
// consequently unreserved/unused for now; hero-broadcastPlaza's relocation
// into holoBazaar (below) is unaffected, that one was never at the hub.
function generateHeroLandmarks()
{
    ...
    return [
        ...
        },
        {
            ...
            rotationY: 0,
            tint: '#28e0ff',
            // Unlike hero-corporateSpire's plain `z + 6` (that district's own gate-spur
            // wedge points away from +z, so it never needed checking), holoBazaar's spur
            // enters from 240deg -- a flat z+6 offset here landed almost exactly on top of
            // a regular building (holoBazaar-1). This angle/radius was chosen by checking
            // clearance against every regular building already placed in this district
            // (min ~3.8 units clear) while staying outside the spur's 35deg-half-angle
            // wedge by a 5deg margin -- see phase-l-implementation-notes.md for the check.
            position: {
                x: findDistrict('holoBazaar').position.x + Math.cos(280 * Math.PI / 180) * 5,
                z: findDistrict('holoBazaar').position.z + Math.sin(280 * Math.PI / 180) * 5
            }
        }
    ]
}
